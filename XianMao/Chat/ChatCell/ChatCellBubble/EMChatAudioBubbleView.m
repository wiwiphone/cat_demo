...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _senderAnimationImages = [[NSMutableArray alloc] initWithObjects: [UIImage imageNamed:SENDER_ANIMATION_IMAGEVIEW_IMAGE_02], [UIImage imageNamed:SENDER_ANIMATION_IMAGEVIEW_IMAGE_03], [UIImage imageNamed:SENDER_ANIMATION_IMAGEVIEW_IMAGE_04], nil];
...
{
    CGFloat width = BUBBLE_VIEW_PADDING*2 + BUBBLE_ARROW_WIDTH + ANIMATION_TIME_LABEL_WIDHT + ((EMVoiceMessageBody *)self.model.firstMessageBody).duration * 2 +
        ANIMATION_TIME_IMAGEVIEW_PADDING + ANIMATION_IMAGEVIEW_SIZE ;
...
-(void)layoutSubviews
{
    ...
    if (self.model.isSender) {
        ...
        frame.origin.y = _animationImageView.center.y - frame.size.height / 2;
        _timeLabel.frame = frame;
        frame.origin.x += frame.size.width - _isReadView.frame.size.width / 2 + ((EMVoiceMessageBody *)self.model.firstMessageBody).duration * 2;
...
- (void)setModel:(EaseMessageModel *)model
{
    ...
    _timeLabel.text = [NSString stringWithFormat:@"%d'",((EMVoiceMessageBody *)self.model.firstMessageBody).duration];
