...
@interface MWPhotoBrowser () <UIActionSheetDelegate>
...
- (void)viewDidLoad {
    ...
    //添加点击手势
    UITapGestureRecognizer *gesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismissOneSelf:)];
    [self.view addGestureRecognizer:gesture];
    //添加长按手势
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(showSaveAction:)];
...
-(void)dismissOneSelf:(UILongPressGestureRecognizer *)gesture{
    
    [self dismissViewControllerAnimated:YES completion:nil];
    
}

-(void)showSaveAction:(UILongPressGestureRecognizer *)longPress{
...
- (void)setCurrentPhotoIndex:(NSUInteger)index {
    ...
    if (photoCount == 0) {
        ...
        if (index >= photoCount)
            index = [self numberOfPhotos];//去掉 -1 修复点击不对称
    }
    _currentPageIndex = index;
	if ([self isViewLoaded]) {
