 ...
 *  @param end    终点坐标
 */
- (void)throwObject:(UIImageView *)obj from:(CGPoint)start to:(CGPoint)end endScale:(CGFloat )endScale
{
 ...
 *  @param path   贝赛尔曲线
 */
- (void)throwObject:(UIImageView *)obj path:(UIBezierPath *)path isRotation:(BOOL)isRotation endScale:(CGFloat)endScale
{
...
{
    [self groupAnimation:path endScale:endScale isRotation:isRotation animationDuration:1.f];
}
