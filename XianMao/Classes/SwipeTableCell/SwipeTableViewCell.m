...
@property (nonatomic, weak) SwipeTableViewCell * currentCell;
@end
...
-(UIView *) hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    BOOL hide = YES;
    if (_currentCell && _currentCell.swipeCellDelegate && [_currentCell.swipeCellDelegate respondsToSelector:@selector(swipeTableCell:shouldHideSwipeOnTap:)]) {
        hide = [_currentCell.swipeCellDelegate swipeTableCell:_currentCell shouldHideSwipeOnTap:p];
    }
    if (hide) {
...
@property (nonatomic, weak) SwipeTableViewCell * cell;
...
@implementation SwipeButtonsView
{
    ...
    SwipeExpansionLayout _expansionLayout;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(SwipeDirection) direction differentWidth:(BOOL) differentWidth
{
...
-(void) expandToOffset:(CGFloat) offset settings:(SwipeExpansionSettings*) settings
{
    ...
    if (!_expandedButton) {
        ...
        [UIView animateWithDuration: duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (_expansionLayout == SwipeExpansionLayoutCenter) {
            ...
            else if (_expansionLayout == SwipeExpansionLayoutNone) {
                [_expandedButton.superview bringSubviewToFront:_expandedButton];
...
#pragma clang diagnostic pop
    ...
    if (_cell.swipeCellDelegate && [_cell.swipeCellDelegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        NSInteger index = [_buttons indexOfObject:sender];
        if (!_fromLeft) {
            index = _buttons.count - index - 1; //right buttons are reversed
        }
        autoHide|= [_cell.swipeCellDelegate swipeTableCell:_cell tappedButtonAtIndex:index direction:_fromLeft ? SwipeDirectionLeftToRight : SwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
-(void) transition:(SwipeTransition) mode percent:(CGFloat) t
{
    switch (mode) {
        ...
        case SwipeTransitionRotate3D: [self transition3D:t]; break;
    }
...
-(instancetype) init
{
    if (self = [super init]) {
        ...
        self.showAnimation = [[SwipeAnimation alloc] init];
        self.hideAnimation = [[SwipeAnimation alloc] init];
        self.stretchAnimation = [[SwipeAnimation alloc] init];
    }
    return self;
}
...
-(instancetype) init
{
    if (self = [super init]) {
        ...
        self.triggerAnimation = [[SwipeAnimation alloc] init];
    }
    return self;
}
@end
...
@property (nonatomic, strong) SwipeAnimation * animation;
...
-(instancetype) init {
    if (self = [super init]) {
        _duration = 0.3;
        _easingFunction = SwipeEasingFunctionCubicOut;
    }
    return self;
}
...
-(void) initViews: (BOOL) cleanButtons
{
    ...
    _firstSwipeState = SwipeStateNone;
...
{
    if (_leftButtons.count == 0 && _swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_swipeCellDelegate swipeTableCell:self swipeButtonsForDirection:SwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_swipeCellDelegate swipeTableCell:self swipeButtonsForDirection:SwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        _leftView = [[SwipeButtonsView alloc] initWithButtons:_leftButtons direction:SwipeDirectionLeftToRight differentWidth:_allowsButtonsWithDifferentWidth];
        _leftView.cell = self;
    ...
    if (!_rightView && _rightButtons.count > 0) {
        _rightView = [[SwipeButtonsView alloc] initWithButtons:_rightButtons direction:SwipeDirectionRightToLeft differentWidth:_allowsButtonsWithDifferentWidth];
...
- (void) showSwipeOverlayIfNeeded
{
    ...
    if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCellWillBeginSwiping:)]) {
        [_swipeCellDelegate swipeTableCellWillBeginSwiping:self];
    }
...
-(void) hideSwipeOverlayIfNeeded
{
    ...
    if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCellWillEndSwiping:)]) {
        [_swipeCellDelegate swipeTableCellWillEndSwiping:self];
    }
...
-(void) prepareForReuse
{
    ...
    [self cleanViews];
    if (_swipeState != SwipeStateNone) {
        _triggerStateChanges = YES;
        [self updateState:SwipeStateNone];
    }
    BOOL cleanButtons = _swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)];
...
-(void) updateState: (SwipeState) newState;
{
    ...
    if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:didChangeSwipeState:gestureIsActive:)]) {
        [_swipeCellDelegate swipeTableCell:self didChangeSwipeState:_swipeState gestureIsActive: self.isSwipeGestureActive] ;
    }
}
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    SwipeSettings * activeSettings = sign < 0 ? _rightSwipeSettings : _leftSwipeSettings;
    ...
    for (int i = 0; i< 2; ++i) {
        ...
        if (expand) {
            ...
            [self updateState:i ? SwipeStateSwipingRightToLeft : SwipeStateSwipingLeftToRight];
        }
    }
}
...
{
    SwipeAnimation * animation = animated ? (_swipeOffset > 0 ? _leftSwipeSettings.hideAnimation: _rightSwipeSettings.hideAnimation) : nil;
...
-(void) showSwipe: (SwipeDirection) direction animated: (BOOL) animated completion:(void(^)(BOOL finished)) completion
{
    [self createSwipeViewIfNeeded];
    ...
    if (buttonsView) {
        ...
        SwipeAnimation * animation = animated ? (direction == SwipeDirectionLeftToRight ? _leftSwipeSettings.showAnimation : _rightSwipeSettings.showAnimation) : nil;
        [self setSwipeOffset:buttonsView.bounds.size.width * s animation:animation completion:completion];
    }
}

-(void) expandSwipe: (SwipeDirection) direction animated: (BOOL) animated
{
    CGFloat s = direction == SwipeDirectionLeftToRight ? 1.0 : -1.0;
    SwipeExpansionSettings* expSetting = direction == SwipeDirectionLeftToRight ? _leftExpansion : _rightExpansion;
    ...
    if(!_activeExpansion && expSetting.fillOnTrigger) {
        ...
        if (buttonsView) {
            __weak SwipeButtonsView * expansionView = direction == SwipeDirectionLeftToRight ? _leftView : _rightView;
            __weak SwipeTableViewCell * weakself = self;
...
{
    SwipeAnimation * animation = animated ? [[SwipeAnimation alloc] init] : nil;
    [self setSwipeOffset:offset animation:animation completion:completion];
}

-(void) setSwipeOffset:(CGFloat)offset animation: (SwipeAnimation *) animation completion:(void(^)(BOOL finished)) completion
{
    if (_displayLink) {
        [_displayLink invalidate];
        _displayLink = nil;
...
{
    BOOL hide = YES;
    if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:shouldHideSwipeOnTap:)]) {
        hide = [_swipeCellDelegate swipeTableCell:self shouldHideSwipeOnTap:[recognizer locationInView:self]];
    }
    if (hide) {
        [self hideSwipeAnimated:YES];
    }
...
-(CGFloat) filterSwipe: (CGFloat) offset
{
    ...
    else if (!_allowsOppositeSwipe && _firstSwipeState == SwipeStateSwipingLeftToRight && offset < 0) {
        offset = 0;
    }
    else if (!_allowsOppositeSwipe && _firstSwipeState == SwipeStateSwipingRightToLeft && offset > 0 ) {
        offset = 0;
    }
    return offset;
}
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (_swipeOffset != 0) {
            _firstSwipeState = _swipeOffset > 0 ? SwipeStateSwipingLeftToRight : SwipeStateSwipingRightToLeft;
        }
        
        if (!_allowsMultipleSwipe) {
            NSArray * cells = [self parentTable].visibleCells;
            for (SwipeTableViewCell * cell in cells) {
                if ([cell isKindOfClass:[SwipeTableViewCell class]] && cell != self) {
        ...
        if (_firstSwipeState == SwipeStateNone) {
            _firstSwipeState = offset > 0 ? SwipeStateSwipingLeftToRight : SwipeStateSwipingRightToLeft;
        }
        self.swipeOffset = [self filterSwipe:offset];
    }
    else {
        __weak SwipeButtonsView * expansion = _activeExpansion;
        if (expansion) {
            __weak UIView * expandedButton = [expansion getExpandedButton];
            SwipeExpansionSettings * expSettings = _swipeOffset > 0 ? _leftExpansion : _rightExpansion;
            ...
            SwipeSettings * settings = _swipeOffset > 0 ? _leftSwipeSettings : _rightSwipeSettings;
            SwipeAnimation * animation = nil;
            if (_targetOffset == 0) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:canSwipe:fromPoint:)]) {
            CGPoint point = [_panRecognizer locationInView:self];
            _allowSwipeLeftToRight = [_swipeCellDelegate swipeTableCell:self canSwipe:SwipeDirectionLeftToRight fromPoint:point];
            _allowSwipeRightToLeft = [_swipeCellDelegate swipeTableCell:self canSwipe:SwipeDirectionRightToLeft fromPoint:point];
        }
        else if (_swipeCellDelegate && [_swipeCellDelegate respondsToSelector:@selector(swipeTableCell:canSwipe:)]) {
            #pragma clang diagnostic push
            #pragma clang diagnostic ignored "-Wdeprecated-declarations"
            _allowSwipeLeftToRight = [_swipeCellDelegate swipeTableCell:self canSwipe:SwipeDirectionLeftToRight];
            _allowSwipeRightToLeft = [_swipeCellDelegate swipeTableCell:self canSwipe:SwipeDirectionRightToLeft];
            #pragma clang diagnostic pop
        }
        else {
            [self fetchButtonsIfNeeded];
