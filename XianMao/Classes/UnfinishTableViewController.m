...
#import "UnfinishTableViewController.h"
...
@interface UnfinishTableViewController () <UITableViewDataSource, UITableViewDelegate, PullRefreshTableViewDelegate, SwipeTableCellDelegate,  UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation UnfinishTableViewController
...
{
    WEAKSELF;
    _dataListLogic = [[DataListLogic alloc] initWithModulePath:@"goods/consignment" path:@"not_completed_goods_list" pageSize:20];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableViewCell isKindOfClass:[SwipeTableViewCell class]]) {
        if (self.status == 33) {
            ...
            ((SwipeTableViewCell*)tableViewCell).swipeCellDelegate = nil;
        }
    }
...
-(void)setPushNews{
    ...
    NSDictionary *param = @{@"user_id" : @([Session sharedInstance].currentUserId), @"goods_sn" : self.goodsId, @"sell_time" : @(self.time)};
...
-(NSArray*)createRightButtons
{
    ...
    UIColor * colors[1] = {[UIColor colorWithHexString:@"ff6666"]};
...
-(BOOL)swipeTableCell:(SwipeTableViewCell*) cell tappedButtonAtIndex:(NSInteger) index direction:(SwipeDirection)direction fromExpansion:(BOOL) fromExpansion {
    ...
    if (direction == SwipeDirectionRightToLeft && index == 0) {
        ...
        NSIndexPath * path = [self.tableView indexPathForCell:cell];
        NSDictionary *dict = [_dataSources objectAtIndex:path.row];
        VerifyModel *verifyModel = [dict objectForKey:@"verifyModel"];
        if (verifyModel && [verifyModel isKindOfClass:[VerifyModel class]]) {
            
            if (verifyModel.goodsStatus == 13 || verifyModel.goodsStatus == 33) {
                [weakSelf showProcessingHUD:nil];
                
                
                
                NSDictionary *parat = @{@"user_id":@([Session sharedInstance].currentUserId), @"goods_sn":verifyModel.goodsSn};
                [[NetworkManager sharedInstance] addRequest:[[NetworkManager sharedInstance] requestWithMethodGET:@"goods/consignment" path:@"delete_goods" parameters:parat completionBlock:^(NSDictionary *data) {
