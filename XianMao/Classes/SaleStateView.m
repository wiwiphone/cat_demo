...
-(VerticalCommandButton *)dSendGoods{
    if (!_dSendGoods) {
        ...
        [_dSendGoods setTitle:@"待上架" forState:UIControlStateNormal];
...
-(VerticalCommandButton *)dDetermine{
    if (!_dDetermine) {
        ...
        [_dDetermine setTitle:@"寄卖中" forState:UIControlStateNormal];
...
-(VerticalCommandButton *)dComeGoods{
    if (!_dComeGoods) {
        ...
        [_dComeGoods setTitle:@"已售出" forState:UIControlStateNormal];
...
-(void)getSendModel:(SendSaleModel *)sendModel{
    self.dSendGoodsLbl.text = [NSString stringWithFormat:@"(%ld)", sendModel.waitFor];
    self.dDetermineLbl.text = [NSString stringWithFormat:@"(%ld)", sendModel.consignment];
    self.dComeGoodsLbl.text = [NSString stringWithFormat:@"(%ld)", sendModel.sellOut];
}
