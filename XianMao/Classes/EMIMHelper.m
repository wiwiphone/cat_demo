...
- (void)loginEasemobSDK
{
    ...
    if (![easemob isLoggedIn] || ([_username length] == 0 || [_password length] == 0)) {
        if ([_username length] == 0 || [_password length] == 0) {
            ...
            _username = [deviceUID stringByReplacingOccurrencesOfString:@"-" withString:@"xianmao"];
            _password = @"";
            
            [easemob asyncRegisterWithUsername:_username password:_password success:^{
                NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
...
//                [easemob.chatManager asyncLoginWithUsername:_username password:_password];
                [easemob registerWithUsername:_username password:_password];
