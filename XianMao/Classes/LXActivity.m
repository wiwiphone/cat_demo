...
//    }
        ...
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i <= shareButtonImagesNameArray.count; i++) {
                ...
                if (self.isHadTitle == YES) {
                    ...
                    [shareButton setFrame:CGRectMake(buttonWidth*(line-1), SHAREBUTTON_INTERVAL_HEIGHT+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+buttonWidth)), buttonWidth, buttonWidth)];

                    NSInteger _stringLength = [[shareButtonTitlesArray objectAtIndex:i-1] length];
                    NSMutableAttributedString *titleAttribute = [[NSMutableAttributedString alloc] initWithString:[shareButtonTitlesArray objectAtIndex:i-1]];
                    
                    UIColor *_black=[UIColor blackColor];
                    UIFont *font=[UIFont fontWithName:@"Helvetica" size:13.0f];
                    [titleAttribute addAttribute:NSFontAttributeName value:font range:NSMakeRange(0, _stringLength)];
                    [titleAttribute addAttribute:NSForegroundColorAttributeName value:_black range:NSMakeRange(0, _stringLength)];
                    [shareButton setAttributedTitle:titleAttribute forState:UIControlStateNormal];
    ...
    if (cancelButtonTitle) {
        ...
        UIView *seperatorLine = [[UIView alloc] initWithFrame:CGRectMake(0,self.LXActivityHeight+BUTTON_INTERVAL_HEIGHT - 2 , kScreenWidth, 1)];
        ...
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight+BUTTON_INTERVAL_HEIGHT, cancelButton.frame.size.width, cancelButton.frame.size.height)];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(0, BUTTON_INTERVAL_HEIGHT, kScreenWidth, BUTTON_HEIGHT)];
...
//    cancelButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    //UIImage *image = [UIImage imageWithColor:[DataSources globalButtonColor]];
    //[cancelButton setBackgroundImage:image forState:UIControlStateNormal];

    NSInteger _stringLength = [cancelButtonTitle length];
    NSMutableAttributedString *titleAttribute = [[NSMutableAttributedString alloc] initWithString:cancelButtonTitle];
    
    UIColor *_black=[UIColor blackColor];
    UIFont *font=[UIFont fontWithName:@"Helvetica" size:15.0f];
    [titleAttribute addAttribute:NSFontAttributeName value:font range:NSMakeRange(0, _stringLength)];
    [titleAttribute addAttribute:NSForegroundColorAttributeName value:_black range:NSMakeRange(0, _stringLength)];
    [cancelButton setAttributedTitle:titleAttribute forState:UIControlStateNormal];
//    [cancelButton setTitle:titleAttribute forState:UIControlStateNormal];
...
//    shareLabel.textColor = [UIColor colorWithHexString:@"#333"];
//    return shareLabel;
//}
