...
@interface VerifyTableViewController () <UITableViewDataSource, UITableViewDelegate, PullRefreshTableViewDelegate, SwipeTableCellDelegate, UIPickerViewDataSource, UIPickerViewDelegate>
...
-(void)setIsThred:(BOOL)isThred{
    _isThred = isThred;
    if (isThred) {
        self.tableView.frame = CGRectMake(0,35, kScreenWidth, kScreenHeight-64-35-35);
    } else {
        self.tableView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight-64-35);
    }
}
...
{
    WEAKSELF;
    _dataListLogic = [[DataListLogic alloc] initWithModulePath:@"goods/consignment" path:@"audit_goods_list" pageSize:20];
