...
+ (PBPhoto *)photoWithImage:(UIImage *)image {
	return [[PBPhoto alloc] initWithImage:image];
}

// Deprecated
+ (PBPhoto *)photoWithFilePath:(NSString *)path {
    return [PBPhoto photoWithURL:[NSURL fileURLWithPath:path]];
}

+ (PBPhoto *)photoWithURL:(NSURL *)url {
	return [[PBPhoto alloc] initWithURL:url];
}
                                                          ...
                                                          progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                                              if (expectedSize > 0) {
                                                                  ...
                                                                  NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:
                                                                                        ...
                                                                                        self, @"photo", nil];
                                                                  [[NSNotificationCenter defaultCenter] postNotificationName:PBPHOTO_PROGRESS_NOTIFICATION object:dict];
                                                              }
                                                          }
...
- (void)postCompleteNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:PBPHOTO_LOADING_DID_END_NOTIFICATION
                                                        object:self];
}
