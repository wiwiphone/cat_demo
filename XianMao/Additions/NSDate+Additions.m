...
static NSString *kNSDateHelperFormatTimeHHMM           = @"yyyy-MM-dd HH:mm";
...
+ (NSString *)stringForTimestampSince1970:(long long)timestamp {
    ...
    return [date stringWithFormat:kNSDateHelperFormatTimeHHMM];
}


- (NSString *)formattedDateRelativeToNow;
{
    NSDateFormatter *midNightDF = [[NSDateFormatter alloc] init];
    [midNightDF setDateFormat:@"yyyy-MM-dd"];
    NSDateFormatter *resultDF = [[NSDateFormatter alloc] init];
    
    NSDate *midnight = [midNightDF dateFromString:[midNightDF stringFromDate:self]];
    
    NSInteger dayDiff = (int)[midnight timeIntervalSinceNow] / (60*60*24);
    NSInteger sencondDiff = (int)[self timeIntervalSinceNow];
    ...
    if (dayDiff == 0 ) {
        ...
        [resultDF setDateFormat:@"yyyy-MM-dd a h':'mm"];
    }
    return [resultDF stringFromDate:self];
}
