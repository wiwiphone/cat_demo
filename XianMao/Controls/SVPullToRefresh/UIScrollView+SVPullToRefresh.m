...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.textColor = [UIColor colorWithHexString:@"c2a79d"];//[UIColor darkGrayColor];
        ...
        UIImage *img = [UIImage imageNamed:@"pull_round_refreshing"];
        _circleImage = [[CALayer alloc] init];
        _circleImage.contents = (id)img.CGImage;
        _circleImage.frame = CGRectMake(0, 0, 20.0f, 20.0f);
        _circleImage.contentsGravity = kCAGravityResizeAspect;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        self.circleImage.position = self.arrow.center;
    }
}
...
- (UIImageView *)arrow {
    if(!_arrow) {
        _arrow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"pull_refresh_arrow"]];
        _arrow.contentMode = UIViewContentModeCenter;
        _arrow.frame = CGRectMake(0, 0, 20.0f, 20.0f);
        _arrow.backgroundColor = [UIColor clearColor];
        [self addSubview:_arrow];
    }
    return _arrow;
