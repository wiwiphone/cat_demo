...
//@property (nonatomic, strong) UIImageView *graduallyImageView;
@end
...
//-(UIImageView *)graduallyImageView{
//    if (!_graduallyImageView) {
//        _graduallyImageView = [[UIImageView alloc] initWithFrame:CGRectZero];
//        _graduallyImageView.image = [UIImage imageNamed:@"gradually-MF"];
//    }
//    return _graduallyImageView;
//}
...
- (void)layoutHeaderViewForScrollViewOffset:(CGPoint)offset
{
    ...
    if (offset.y > 0)
    {
        ...
        CGFloat f = 1 / kDefaultHeaderFrame.size.height * offset.y * 2;
        if (f>0.4)f=0.4f;
        self.bluredImageView.alpha = f;//  1 / kDefaultHeaderFrame.size.height * offset.y * 2;
    ...
    else
    {
        ...
        rect = kDefaultHeaderFrame;
        rect.origin.y += delta/2;
        self.innerView.frame = rect;
        
        self.bluredImageView.alpha = 0.f;
    }
}
...
- (void)setHeaderView:(UIView *)headerView {
    if (_innerView!=headerView && self != headerView) {
        [_innerView removeFromSuperview];
        _innerView = headerView;
        _innerView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        
        _innerView.contentMode = UIViewContentModeScaleAspectFill;
        
        _innerView.frame = self.imageScrollView.bounds;
        
        [self.imageScrollView addSubview:_innerView];
        [self.imageScrollView bringSubviewToFront:self.bluredImageView];
        [self.imageScrollView bringSubviewToFront:self.headerTitleLabel];
        [self refreshBlurViewForNewImage];
        
//        [self.imageScrollView addSubview:self.graduallyImageView];
//        [self setUpUI];
//        [self.imageScrollView bringSubviewToFront:self.graduallyImageView];
    }
}

//- (void) setUpUI {
//    [self.graduallyImageView mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.bottom.equalTo(self.innerView.mas_bottom);
//        make.left.equalTo(self.innerView.mas_left);
...
//        self.bluredImageView.image = screenShot;
    self.bluredImageView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:1.f];
    self.bluredImageView.alpha = 0.f;
}
