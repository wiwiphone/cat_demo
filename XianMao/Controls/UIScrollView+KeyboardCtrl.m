//
//  UIScrollView+KeyboardCtrl.m
//  Fisherman
//
//  Created by simon cai on 7/29/14.
...
@interface UIScrollView (KeyboardCtrlEx)

@property (nonatomic, assign) CGFloat previousKeyboardY;

@end

@implementation UIScrollView (KeyboardCtrl)
...
- (void)setupPanGestureControlKeyboardHide:(BOOL)isPanGestured {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleKeyboardDidShowHideNotification:)
...
- (void)handleKeyboardDidShowHideNotification:(NSNotification *)notification {
    BOOL didShowed = YES;
...
- (void)keyboardWillShowHide:(NSNotification *)notification {
    ...
    //[self animationOptionsForCurve:curve]; //UIViewAnimationOptionAllowAnimatedContent; //
    
    if (self.keyboardWillChange) {
        self.keyboardWillChange(beginRect,endRect, options, duration, (([notification.name isEqualToString:UIKeyboardWillShowNotification]) ? YES : NO));
    }
}
...
- (CGRect) fixKeyboardRect:(CGRect)originalRect{
    ...
    referenceView = appDelegate.window;
