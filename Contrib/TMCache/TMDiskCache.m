...
- (void)initializeDiskProperties
{
    ...
    NSArray *keys = @[ NSURLContentModificationDateKey, NSURLFileAllocatedSizeKey ];
    ...
    for (NSURL *fileURL in files) {
        ...
        NSNumber *fileSize = [dictionary objectForKey:NSURLFileAllocatedSizeKey];
        if (fileSize) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(TMDiskCacheObjectBlock)block
{
    ...
    dispatch_async(_queue, ^{
        ...
        if (written) {
            ...
            error = nil;
            NSDictionary *values = [fileURL resourceValuesForKeys:@[ NSURLFileAllocatedSizeKey ] error:&error];
            TMDiskCacheError(error);

            NSNumber *diskFileSize = [values objectForKey:NSURLFileAllocatedSizeKey];
            if (diskFileSize) {
