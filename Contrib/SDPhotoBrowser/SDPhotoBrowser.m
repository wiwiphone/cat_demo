...
- (void)setupToolbars
{
    ...
    if (self.goodsId || self.goodsId.length > 0) {
        UIButton * goods = [[UIButton alloc] init];
        [goods setTitle:@"商品详情" forState:UIControlStateNormal];
        [goods setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        goods.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
        goods.layer.cornerRadius = 5;
        goods.clipsToBounds = YES;
        [goods addTarget:self action:@selector(goodsButtonClick) forControlEvents:UIControlEventTouchUpInside];
        _goods = goods;
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        ...
        UILongPressGestureRecognizer * longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPresspgr:)];
...
-(void)longPresspgr:(UILongPressGestureRecognizer *)longPress
{
    if (longPress.state == longPress.enabled) {
...
- (void)layoutSubviews
{
    ...
    _goods.frame = CGRectMake((kScreenWidth-100)/2, self.bounds.size.height - 70, 100, 25);
