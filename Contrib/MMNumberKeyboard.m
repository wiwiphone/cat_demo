...
- (void)_dismissKeyboard:(id)sender
{
    ...
    if (firstResponder) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(numberKeyboardShoulddismiss:)]) {
            [self.delegate numberKeyboardShoulddismiss:self];
        }
    }
}
...
- (void)_buttonStyleDidChange
{
    ...
    if (style == MMNumberKeyboardButtonStyleWhite) {
        ...
        highlightedFillColor = [UIColor whiteColor];
    }else if (style == MMNumberKeyboardButtonStyleRed) {
        fillColor = [UIColor colorWithRed:0.97 green:0.22f blue:0.28 alpha:1];
        highlightedFillColor = [UIColor whiteColor];
    }
    
    UIColor *controlColor = nil;
    UIColor *highlightedControlColor = nil;
    if (style == MMNumberKeyboardButtonStyleDone || style == MMNumberKeyboardButtonStyleRed) {
