...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self initialization];
//        _mainView.contentInset = UIEdgeInsetsMake(0, (kScreenWidth-(kScreenWidth-40))/2, 0, -(kScreenWidth-(kScreenWidth-40))/2);
    }
    return self;
}
...
-(void)setRedUrl:(NSArray *)redUrl{
    _redUrl = redUrl;
}

-(void)setTitleAndSubtitles:(NSMutableArray *)titleAndSubtitles{
    _titleAndSubtitles = titleAndSubtitles;
...
- (void)layoutSubviews
{
    ...
    if (self.isMain) {
        _flowLayout.itemSize = CGSizeMake(self.width-40, self.height);
    } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.isMianView = self.isMain;
    
    NSString *imagePath = self.imagePathsGroup[itemIndex];
    NSString *redUrl = self.redUrl[itemIndex];
    ...
    NSDictionary *dict = [self.titleAndSubtitles objectAtIndex:indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtUrl:)]) {
        [self.delegate cycleScrollView:self didSelectItemAtUrl:cell.redUrl];
    }
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(cycGetCurrPage:TotoaPage:)]) {
        [self.delegate cycGetCurrPage:(_offer/(kScreenWidth-30))+1 TotoaPage:self.imagePathsGroup.count];
    }
}
    ...
    if (self.isMain) {
        if (fabs(scrollView.contentOffset.x -_offer) > 20) {
            if (scrollView.contentOffset.x > _offer) {
                ...
                if (i > _totalItemsCount-1) {
                    i = (int)_totalItemsCount-1;
                }
                NSIndexPath * index =  [NSIndexPath indexPathForRow:i inSection:0];
                [_mainView scrollToItemAtIndexPath:index atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:YES];
            }else{
                int i = scrollView.contentOffset.x/([UIScreen mainScreen].bounds.size.width - 30)+1;
                if (i < 1) {
                    i = 1;
