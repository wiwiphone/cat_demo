...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    if(!self.overlayView.superview){
        ...
        // Ensure that overlay will be exactly on top of BaseViewController (which may be changed during runtime).
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0.0f || self.hudView.alpha == 0.0f) {
                             ...
                             // Tell the BaseViewController to update the StatusBar appearance
#if !defined(SV_APP_EXTENSIONS)
