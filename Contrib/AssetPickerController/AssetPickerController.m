...
#define kThumbnailLength    78.0f // 100.0f
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"AssetPicker.Bundle/Images/%@@2x.png",(!IS_IOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
{
    self.asset=asset;
//    if (asset == [UIButton class]) {
//        [_imageView addSubview:(UIButton *)asset];
//    } else {
...
- (void)bind:(NSArray *)assets selectionFilter:(NSPredicate*)selectionFilter minimumInteritemSpacing:(float)minimumInteritemSpacing minimumLineSpacing:(float)minimumLineSpacing columns:(int)columns assetViewX:(float)assetViewX{
    ...
    if (self.contentView.subviews.count<assets.count) {
        for (int i=0; i<assets.count; i++) {
            if (i>((NSInteger)self.contentView.subviews.count-2)) {
//                NSLog(@"%d, %ld", i, self.contentView.subviews.count - 2);
...
@property (nonatomic, weak) UIButton *cammerBtn;
...
//    UIButton *cammerBtn = [[UIButton alloc] initWithFrame:CGRectZero];//(((self.tableView.frame.size.width-kThumbnailSize.width*(ceil((self.assets.count)*1.0/columns)+1)-minimumInteritemSpacing*((ceil((self.assets.count)*1.0/columns)+1)-1))/2)+(kThumbnailSize.width+minimumInteritemSpacing), 1, kThumbnailLength, kThumbnailLength)];
//    cammerBtn.backgroundColor = [UIColor redColor];
//    [self.view addSubview:cammerBtn];
//    [cammerBtn addTarget:self action:@selector(clickCammerBtn) forControlEvents:UIControlEventTouchUpInside];
//    self.cammerBtn = cammerBtn;
...
-(void)clickCammerBtn{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cammer" object:nil];
}
...
//        ALAsset *ass = [[ALAsset alloc] init];
//        [self.assets addObject:ass];
    }
    else {
    ...
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        ...
        if (asset)
        {
            ...
            if ([type isEqual:ALAssetTypePhoto])
                weakSelf.numberOfPhotos ++;
            if ([type isEqual:ALAssetTypeVideo])
                weakSelf.numberOfVideos ++;
        }
        ...
        else if (weakSelf.assets.count > 0)
        {
            ...
            [weakSelf.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:ceil(weakSelf.assets.count*1.0/columns)  inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:YES];
        }
    };
...
//    if (indexPath.row == 0) {
//        self.cammerBtn.frame = CGRectMake(((self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-minimumInteritemSpacing*(tempAssets.count-1))/2)+(kThumbnailSize.width+minimumInteritemSpacing)*0, minimumLineSpacing-1, kThumbnailSize.width, kThumbnailSize.height);
//    }
    static NSString *CellIdentifier = kAssetViewCellIdentifier;
    AssetPickerController *picker = (AssetPickerController *)self.navigationController;
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset
{
    AssetPickerController *vc = (AssetPickerController *)self.navigationController;
...
- (void)showNotAllowed
{
    ...
    UIImageView *padlock    = [[UIImageView alloc] initWithImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"AssetPicker.Bundle/Images/AssetsPickerLocked@2x.png"]]];
...
- (void)showNoAssets
{
    ...
    message.text            = NSLocalizedString(@"你可以使用 iTunes 将照片和视频\n同步到 iPhone。", nil);
