...
@property (nonatomic, strong) UIButton *goGoodsDetail;
@end

@implementation MJPhotoBrowser

-(UIButton *)goGoodsDetail{
    if (!_goGoodsDetail) {
        _goGoodsDetail = [[UIButton alloc] initWithFrame:CGRectZero];
        [_goGoodsDetail setTitle:@"商品详情" forState:UIControlStateNormal];
        [_goGoodsDetail setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        _goGoodsDetail.titleLabel.font = [UIFont systemFontOfSize:14.f];
        _goGoodsDetail.backgroundColor = [UIColor whiteColor];
        _goGoodsDetail.layer.masksToBounds = YES;
        _goGoodsDetail.layer.cornerRadius = 3;
        [_goGoodsDetail sizeToFit];
    }
    return _goGoodsDetail;
}
...
- (void)loadView
{
    ...
    // 隐藏状态栏
    //[[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:UIStatusBarAnimationFade]; //先去掉，隐藏后显示会有问题［我的头像］
...
- (void)viewDidLoad
{
    ...
    if (self.isHaveGoodsDetailBtn == 0) {
        [self.view addSubview:self.goGoodsDetail];
        [self.goGoodsDetail mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self.goGoodsDetail addTarget:self action:@selector(clickGoGoodsDetail) forControlEvents:UIControlEventTouchUpInside];
    }
    
}

-(void)clickGoGoodsDetail{
    ...
    [[CoordinatingController sharedInstance] pushViewController:detailViewController animated:YES];
...
{
    NSInteger currentPhotoIndex = _photoScrollView.contentOffset.x / _photoScrollView.frame.size.width;
    if (currentPhotoIndex != _currentPhotoIndex) {
